...
public class AgendaDao {
	...
	public void registrarAgenda(Agenda ag) throws SQLException, Exception {
		...
		try {
			...
			java.sql.PreparedStatement sqlPrep = Conexao.conectar().prepareStatement(sql);
			sqlPrep.setInt(1, RetornarProximoCodigoAgenda()-1); //-1 por que esse deve ser o atual, já que ja foi criado
	 ...
	 * ####AJUSTAR PARA ATUALIZAR SERVIÇO TAMBÉM
	 * ####DEVE-SE PODER ESCOLHER MAIS DE UM SERVIÇO POR AGENDA, DE ACORDO COM O BANCO 
	 * @param ag
	 * @throws Exception
	 */
	...
	public List<Object> buscarTodos(String data) throws SQLException, Exception {
		...
		try {
			...
			//Altera resultado para sim ou não ao invés de mostrar 0 e 1 como no banco
			String status="";			
			while (rs.next()) {
	...
	/**
	 * Altera o estado da agenda (realizado ou não) e registra no caixa a entrada
	...
	public void toogleRealizado(Agenda ag) {
		...
		if (ag.isRealizado()) {
			...
			try {
				...
				java.sql.PreparedStatement sqlPrep = Conexao.conectar().prepareStatement(sql);
				sqlPrep.setInt(1, ag.getCodigo());
				sqlPrep.setInt(2, cxDao.RetornarProximoCodigoRegistro()-1); //-1 pra pegar código atual já registrado
