...
public class ClCadastrarCliente extends JInternalFrame {
	...
	public static MaskFormatter mascaraField(String Mascara) {
		...
		try {
			F_Mascara.setMask(Mascara); // Atribui a máscara
	...
	/**
	 * #### (Fazer outras validações) Realiza a validação dos campos
	 * De forma alguma deve ser cadastrado cliente sem um telefone senão não mostrará na tabela
	 */
	public boolean validado() {
		if (!tfContato.getText().equals("(  )      -    ") && !tfNome.getText().equals("") && !tfCpf.getText().equals("   .   .   -  ") && !tfLogradouro.getText().equals("") && dcNascimento.getDate() != null){
			...
			JOptionPane.showMessageDialog(null, "Por favor preencha todas as informações necessárias", "Aviso", JOptionPane.WARNING_MESSAGE);
			return false;
		}
	}
}